...
const calculateMentions = (video: any) => {
  return Math.floor(
    (video.like_count + video.comment_count + video.share_count) / 100
  );
};

// 获取趋势数据
const fetchTrendData = async (keyword: string = "Q音") => {
  try {
    ...
    if (Array.isArray(data)) {
      ...
      const latestVideo = data.reduce((latest: any, current: any) => {
        ...
        return !latest || currentDate > new Date(latest.created_at)
          ? current
          : latest;
      }, null);
